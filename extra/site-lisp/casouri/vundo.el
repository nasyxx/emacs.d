...
;; back.
;;
;; If you move around in the vundo buffer, go to the original buffer,
;; make some change and come back, the vundo buffer is not updated
;; automatically. Then if you invoke any vundo command, vundo doesn't
;; perform that command but instead updates the buffer to catch up
;; with the original buffer. After that, if you still want to perform
;; that command, invoke it again.
...
;; and draw it in the buffer. We have two data: `vundo--prev-mod-list'
;; which stores a list of `vundo-m'. This list is directly generated
;; from `buffer-undo-list' by `vundo--mod-list-from'. We also have a
;; hash table `vundo--prev-mod-hash' generated by
;; `vundo--update-mapping', which maps undo-lists to the `vundo-m'
;; object corresponding to it. Once we have the mod-list and hash
;; table, we connect the nodes in mod-list to form a tree in
  ...
  (if vundo-ascii-mode
      (setq vundo-translation-alist
            '((?○ . ?o)
              ...
              (?├ . ?|)
              (?└ . ?+)))
    (setq vundo-translation-alist nil)))
...
If FROM non-nil, build from FORM-th modification in MOD-LIST."
  (cl-loop
   ...
   do (let ((prev-undo (undo--last-change-was-undo-p
        ...
        (pcase prev-undo
          ;; This is an ordinary undo. Merge it with its equivalent
          ;; nodes.
          ((pred consp)
      ...
      (if (> (length undo-list) (length vundo--prev-undo-list))
        ...
        (let ((ul undo-list))
          ...
          (if-let* ((new-tail (gethash ul vundo--prev-mod-hash))
            ...
            (error "Couldn't find the modification"))))
...
Do sanity check, then evaluate BODY."
  `(progn
     ...
     ;; then comes back), refresh and catch up.

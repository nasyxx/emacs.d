...
#+begin_src elisp
  ...
  (defun nasy/set-default-font (charset &optional size frame)
    "Set CHARSET fontset to default *font* with SIZE on FRAME."
    (when (and (display-graphic-p)
              (nasy/check-font *font*))
      (let ((size (or size *font-size*)))
        (set-fontset-font t
                          charset
  ...
  Font for char like 𨉚."
    (when (display-graphic-p)
      (let ((size (or size *font-size-cjk*)))
  ...
  (defun nasy/set-symbol (charset &optional size weight frame)
    ...
    Font for char like ∷."
      (when (display-graphic-p)
        (let ((size (or size *font-size-symbol*)))
          (when (nasy/check-font *font-symbol*)
            (set-fontset-font (frame-parameter nil 'font)
                              charset

...
#+begin_src elisp :tangle (concat (file-name-directory (buffer-file-name)) "config/nasy-config.el")
  ...
  (defvar   *risky-var* t)  ;; If nil, disable risky-local-variable
...
#+begin_src elisp :tangle (concat (file-name-directory (buffer-file-name)) "config/nasy-config.el")
  ...
  (when *is-a-mac*
    ...
    (add-to-list 'default-frame-alist
                 '(font . "OperatorMonoSSmLig Nerd Font-12"))
...
I use general.el to set some variables, but key-bindings.
...
#+begin_src elisp
  ;; (use-package eldoc
  ;;   :hook ((org-mode . (lambda () (eldoc-mode 1)))))
#+end_src
...
#+begin_src elisp
  (use-package hydra
    :straight t
    :bind (("C-x 9" . hydra-unicode/body)
           ("C-x |" . hydra-window/body)
           :map dired-mode-map
           ("." . hydra-dired/body))
    :config
    ;; insert unicode
...
#+begin_src elisp
  (use-package counsel
    ...
    :bind (("M-x"   . counsel-M-x)
           ...
           ("C-c C-a o" . counsel-outline)
           ("C-c C-a i" . counsel-imenu)
           ("C-c C-a p" . counsel-list-processes)
           ("C-c C-a d" . counsel-dash)
           ("C-c C-a u" . counsel-unicode-char)
    ...
    :config (gsetq counsel-yank-pop-height ivy-height)
...
#+begin_src elisp
  (use-package all-the-icons-ivy
    ...
    :straight t
    :init (setq all-the-icons-ivy-file-commands
                '(counsel-find-file
                  counsel-file-jump
                  counsel-recentf
                  counsel-projectile
...
#+begin_src elisp
  (use-package outline
    :disabled t
    :hook (((prog-mode conf-mode bibtex-mode LaTeX-mode) . outline-minor-mode))
...
#+begin_src elisp
  (use-package transient
    :straight t
    :init (setq transient-default-level 5))
#+end_src
...
#+begin_src elisp
  ...
  When ARG is non-nil do not include decorators."
    ...
    :bind (:map python-mode-map
                ("C-c C-b" . nasy:python-send-buffer)
                ("C-c C-r" . nasy:python-send-region)
                ("C-c C-c" . nasy:python-send-defun)
                ("C-c C-z" . nasy:python-switch-to-shell))
    :hook ((python-mode . lsp)
           (python-mode . delete-company-lsp)
    ...
    (with-eval-after-load 'lsp-pyls
      (setq lsp-pyls-configuration-sources                          ["pydocstyle" "pycodestyle" "pyflakes" "pylint"]

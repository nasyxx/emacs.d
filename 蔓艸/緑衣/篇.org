#+PROPERTY: header-args:elisp :tangle (concat temporary-file-directory "緑衣.el") :lexical t
#+title: 緑衣
...
*** Autoloads
:PROPERTIES:
:header-args:elisp: :tangle (concat temporary-file-directory "nasy-company.el") :lexical t
:END:
  ...
  (n/leaf-nv recentf
    ...
    (recentf-exclude
     . `(,(abbreviate-file-name
         ...
         "/tmp/" "/ssh:" "/nix/store" "~/.nix/store" "~/.emacs.d/straight")))
#+end_src
...
#+begin_src elisp
  (n/leaf-nv session
    :hook
    (emacs-startup-hook . session-initialize)
    :custom
